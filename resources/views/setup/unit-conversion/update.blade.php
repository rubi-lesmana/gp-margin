...
@section('content')
    <div class="content-wrapper">
        ...
        <form action="{{ route('unit-conversions.update', $unitConversion->id_unit_conversion) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control" name="id_unit_conversion" disabled
                                       value="{{ $unitConversion->id_unit_conversion }}" />
                            </div>

                            <div class="mt-3">
                                <label class="form-label">Description</label>
                                <textarea class="form-control" name="description" placeholder="Description"
                                          rows="3">{{ old('description', $unitConversion->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div id="detailWrapper">
                                @php
                                    // Fallback: gunakan old input jika validasi sebelumnya gagal,
                                    // supaya baris yang sudah diisi user tidak hilang.
                                    $oldUnitIds     = old('unit_id', $unitConversion->details->pluck('unit_id')->toArray());
                                    $oldConversions = old('conversion_value', $unitConversion->details->pluck('conversion_value')->toArray());
                                @endphp

                                @foreach ($oldUnitIds as $i => $selectedUnitId)
                                        ...
                                        <div class="col-12 col-md-4 mb-3 mb-md-0">
                                            ...
                                            <select class="form-select select2" name="unit_id[]" required>
                                                ...
                                                @foreach($unit as $description => $unit_id)
                                                    <option value="{{ $unit_id }}" @selected($unit_id == $selectedUnitId)>
                                                        {{ $description }}
                                                    </option>
                                                @endforeach
                                            </select>
                            ...
                            <div class="d-flex flex-column flex-sm-row gap-2 mt-3">
                                <button type="submit" class="btn btn-primary w-sm-auto">Update</button>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Init select2 untuk baris-baris existing yang sudah ada di DOM (hasil edit)
        initSelect2(wrapper);
